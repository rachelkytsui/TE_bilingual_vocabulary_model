...
Correlation <- keepers_ws_TE %>%
  select(age_continuous, balance_vocab, word_vocab, TotalNW_Dom, TotalNW_NonDom,
...
corrplot(mycor)
write.csv(mycor, "ObservedTE_correlations.csv", col.names=NA)
...
###### Stacked barchat: Y=Total vocab, X= individual baby (stacked: TE, unique dom, unique nondom) ######
```{r}

##Creating dataset for stacked barchart
...
##Stacked barchart (sort by total number of TEs)

stacked %>%
  ggplot(aes(x = reorder(baby_id, -TE, sum), y = NumberOfWords)) +
  geom_col(aes(fill = Type)) +
    labs(x = "Individual infant", 
       y = "Number of words produced") + 
  scale_fill_manual(values = c("#1f78b4", "#a6cee3", "#33a02c"))
...
#stacked %>%
#  mutate(bin_balance = cut_width(balance_vocab, 0.333333333333333, boundary = 0)) %>%
#  ggplot(aes(x = reorder(Baby_ID, -NumberOfWords, sum), y = NumberOfWords)) +
#  geom_col(aes(fill = Type)) +
#  facet_grid(. ~ bin_balance, labeller=labeller(bin_balance = balance_facet)) + 
#  labs(x = "Individual infant", 
#       y = "Number of words produced") + 
#  scale_fill_manual(values = c("#1f78b4", "#a6cee3", "#33a02c"))

Bal_StackedPlot <- stacked %>%
  mutate(bin_B = cut(balance_vocab, breaks = 2, 
                     labels = c("More balanced", "Less balanced"))) %>%
  ggplot(aes(x = reorder(baby_id, -NumberOfWords, sum), y = NumberOfWords)) +
  geom_col(aes(fill = Type)) +
  facet_grid(. ~ bin_B) +
  scale_fill_manual(values = c("#1f78b4", "#a6cee3", "#33a02c")) +
...
stacked <- keepers_ws_TE %>%
  ...
  mutate(BalSubset = cut(balance_vocab, breaks = 3, 
                         labels=c("HighBalance", "MidRangeBalance", "LeastBalance")))


LeastBal_plot <- stacked %>%
  filter(BalSubset == "LeastBalance") %>%
  ggplot(aes(x = reorder(baby_id, -NumberOfWords, sum), y = NumberOfWords)) +
  geom_col(aes(fill = Type)) +
  scale_fill_manual(values = c("#1f78b4", "#a6cee3", "#33a02c")) +
  scale_y_continuous(limits = c(0, 1050)) +
...
MidRangeBal_plot <- stacked %>%
  filter(BalSubset == "MidRangeBalance") %>%
  ggplot(aes(x = reorder(baby_id, -NumberOfWords, sum), y = NumberOfWords)) +
  geom_col(aes(fill = Type)) +
  scale_fill_manual(values = c("#1f78b4", "#a6cee3", "#33a02c")) +
  scale_y_continuous(limits = c(0, 1050)) +
...
HighBal_plot <- stacked %>%
  filter(BalSubset == "HighBalance") %>%
  ggplot(aes(x = reorder(baby_id, -NumberOfWords, sum), y = NumberOfWords)) +
  geom_col(aes(fill = Type)) +
  scale_fill_manual(values = c("#1f78b4", "#a6cee3", "#33a02c")) +
  scale_y_continuous(limits = c(0, 1050)) +
...
Bal_StackedPlot <- ggarrange(LeastBal_plot, 
          MidRangeBal_plot + rremove("ylab"), 
          HighBal_plot + rremove("ylab"),
          ncol = 3, nrow = 1,
...
## Linear regression: TE ~ B + D + N
```{r}
TE_input_lm <- lmer(number_of_te ~ 0 + balance_vocab + TotalNW_Dom + TotalNW_NonDom,
                  data = keepers_ws_TE)

summary(TE_input_lm)
```

## Linear regression: TE ~ Du * Nu
```{r}
TE_output_lm <- lm(number_of_te ~ 0 + Dom_unique_words * NonDom_unique_words,
                  data = keepers_ws_TE)

summary(TE_output_lm)

### plot the Du*Nu interaction
#effect_plot(TE_InputConstant, pred = B, plot.points = TRUE)
interaction_DuNu <- effect(term = "Dom_unique_words:NonDom_unique_words", mod = TE_output_lm) %>%
  data.frame()

plot_interactionDuNu <- ggplot(interaction_DuNu, 
                               aes(x = Dom_unique_words, y = fit, group = NonDom_unique_words)) +
  geom_line(aes(color = NonDom_unique_words)) +
  labs(x = 'Unique dominant words',
       y = 'Number of TEs',
       color = 'Unique non-dominant words')
...
#  facet_grid(. ~TEgroup) +
  labs(x = "Simulated total number of TEs (based on percentile)", y = "Observed total number of TEs") +
  theme(plot.title = element_text(hjust = 0.5)) +
  geom_smooth(method = "lm", se = F) +
  geom_point(shape = 23) +
  geom_abline(linetype = 2, alpha = 0.5) +
...
ggsave(here("Output/TEprediction_percentile&CDI.png"), TEprediction_percentile_CDI)
...
ggsave(here("Output/TEprediction_percentile&CDI_LessThan100.png"), TEprediction_percentile_CDI_LessThan100)
...
ggsave(here("Output/TEprediction_percentile&CDI_LessThan50.png"), TEprediction_percentile_CDI_LessThan50)
...
ggsave(here("Output/TEprediction_percentile&CDI_MoreThan100.png"), TEprediction_percentile_CDI_MoreThan100)


##### Facet by W (< 100 vs. > 100)
