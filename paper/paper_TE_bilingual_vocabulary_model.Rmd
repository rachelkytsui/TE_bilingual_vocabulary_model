...
simulation2 <- data_simulation2 %>%
  ...
  mutate(vocab_type = factor(vocab_type, levels=c("CONCEPT", "TE", "DOM_SINGLET", "NONDOM_SINGLET")),
         vocab_type = recode(vocab_type, 
                             ...
                             DOM_SINGLET = "Panel 1C: \n Singlets in dominant language \n (DOM_SINGLET)",
                             NONDOM_SINGLET = "Panel 1D: \n Singlets in non-dominant language \n (NONDOM_SINGLET)")) %>%
...
Observed_plot_LEARNABLE <- keepers_ws_TE %>%
  ...
  mutate(vocab_type = factor(vocab_type, levels=c("concept_vocab", "number_of_te", "total_singlet_dom", "total_singlet_nondom")),
         vocab_type = recode(vocab_type, 
                             ...
                             total_singlet_dom = "Panel 2C: \n Singlets in dominant language \n (DOM_SINGLET)",
                             total_singlet_nondom = "Panel 2D: \n Singlets in non-dominant language \n (NONDOM_SINGLET)")) %>%
...
kable(ws_percentile,
        ...
        caption = "Table 6. The number of total English and French productive CDI vocabulary at the 90th percentile at different ages, and the average between the two which serves as the denominator in our computation model. Values are rounded to the nearest integer.") %>%
...
_ObservedTE * 90th percentile of CDI items ~ 0 + Dominant vocabulary:Non-dominant vocabulary_)
...
Our model showed an excellent model fit of $R^2$ = `r round(summary(test_prediction3_model)$r.squared,2)`, indicating that our model explained `r round(summary(test_prediction3_model)$r.squared,2) * 100`% of the variance in bilinguals’ translation equivalent knowledge. The linear regression model estimated a BIAS coefficient of `r round(summary(test_prediction3_model)$coefficient[1],2)`, $p `r papaja::printp(summary(test_prediction3_model)$coefficient[4])`$. This value is close to 1, suggesting that our data are consistent with the account whereby translation equivalents are learned equivalently to other words[^4].

[^4]: We also explored how these results would vary if we set the number of LEARNABLE words at the 75th percentile, rather than at the 90th percentile. Overall, model fit was slightly reduced compared to our main model, although fit was still good, $R^2$ = `r round(summary(test_prediction3_model_75percentile)$r.squared,2)`, $p `r papaja::printp(summary(test_prediction3_model_75percentile)$coefficient[4])`$. The estimated BIAS parameter was lower, at `r round(summary(test_prediction3_model_75percentile)$coefficient[1],2)`, which is nonetheless still quite close to 1, and thus consistent with the Neutral Account. This change in the BIAS parameter is mathematically expected due to the reciprocal nature of LEARNABLE and BIAS, an issue we will return to in the discussion.
