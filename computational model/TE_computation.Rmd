...
W <- seq(0, 2*V, by = 25) %>% # a sequence of 0 to 2V
  rep(., 21) # generate 21 repetitinos of the sequence

# Balance, i.e. proportion use of non-dominant language (B ≤ .50)
B <- seq(0.5, 1, by = 0.025) %>% # a sequence of 0.5 to 1
  rep(., 49) # generate 25 repetitions of the sequence
...
D <- seq(0, V, by = 25) %>% # Words produced in the dominant language
  rep(., 25) # generate 13 repetitions of the sequence

N <- seq(0, V, by = 25) %>% # Words produced in non-dominant language
...
## Creating variables: D and N
D <- seq(0, V, by = 25) %>% # Words produced in the dominant language
  rep(., 21) # generate 13 repetitions of the sequence

B <- seq(0.5, 1, by = 0.025) %>% # a sequence of 0.5 to 1
  rep(., 25) # generate 25 repetitions of the sequence

## Creating the dataframe
TE_fromVDB <- data.frame(V, D) %>%
  arrange(D) %>%
  mutate(B = B) %>%
  # Create other variables
...
## Creating variables: D and N
N <- seq(0, V, by = 25) %>% # Words produced in the dominant language
  rep(., 21) # generate 13 repetitions of the sequence
